...
def send_mail(from_name, old):
    text = f"ich habe nicht verstanden. Regards {from_name} i am only {old} years "
    print(text)


send_mail("Oleksandr", 7 )
...
def have_duplicates(array):
    have_duplicates = not (len(set(array)) == len(a))
...
def is_valid_pin_codes(pincodes):
    ...
    if not pincode.isdigit():
        is_valid = False
    is not isinstance(pincode, str)

return is is_valid



a = ["111", "567893", "1535627"] 
print(is_valid_pin_codes(a))




numbers =["123", "456", "1000", "fghj!"]
...
def transform_to_integers(number):
    result = list()
    for number in numbers:
